...
void LogicControl::processKeyEvent(QKeyEvent *event)
{
...
void LogicControl::writeContentToPngFile(const QString &fileName)
{
    ...
    m_graphWidget->scene()->setBackgroundBrush(WidgetControl::m_paperColor);
...
void LogicControl::setActiveNode(Node *node)
{
    if (m_activeNode!=0)
...
//    Edge *edge = new Edge(m_activeNode, node);
//    m_graphWidget->scene()->addItem(edge);
////    m_nodeList.append(node);
//    edge->setColor(node->color());
//    edge->setWidth(node->scale()*2 + 1);
//    edge->setSecondary(false);
...
void LogicControl::selectNode(Node *node)
{
    // leave hint mode
//    showingAllNodeNumbers(false);
//    m_showingNodeNumbers = false;
...
QList<Edge *> LogicControl::allEdges() const
{
    QList<Edge *> list;
...
void LogicControl::addEdge(Node *source, Node *destination)
{
...
void LogicControl::removeEdge(Node *source, Node *destination)
{
    if (!source->isConnected(destination))
    {
